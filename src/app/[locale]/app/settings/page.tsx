...
export default function SettingsPage() {
  ...
  const Section = ({ titleEn, titleAr, icon: Icon, children }: {
    ...
    <div className="rounded-2xl border border-[var(--foreground)]/[0.06] bg-[var(--foreground)]/[0.02] overflow-hidden">
      <div className="flex items-center gap-3 p-5 border-b border-[var(--foreground)]/[0.06]">
  ...
  const Field = ({ labelEn, labelAr, children }: { labelEn: string; labelAr: string; children: React.ReactNode }) => (
    <div className="flex items-center justify-between gap-4">
      <span className="text-xs text-[var(--foreground)]/60">{isAr ? labelAr : labelEn}</span>
      {children}
    </div>
  );
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6 pb-20 max-w-[800px] mx-auto">
      <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0} className="mb-6">
        ...
        <p className="text-sm text-[var(--foreground)]/50 mt-1">{isAr ? 'تخصيص تجربتك' : 'Customize your experience'}</p>
      </motion.div>
      ...
      <motion.div initial="hidden" animate="visible" className="space-y-6">
          ...
          <Section titleEn="Profile" titleAr="الملف الشخصي" icon={User}>
            <Field labelEn="Display Name" labelAr="اسم العرض">
              <input value={s.displayName}
                ...
                className="w-48 rounded-lg border border-[var(--foreground)]/[0.08] bg-transparent px-3 py-1.5 text-xs text-end focus:outline-none focus:border-[var(--color-primary)]/40" />
            </Field>
          </Section>
        </motion.div>
          ...
          <Section titleEn="Preferences" titleAr="التفضيلات" icon={Settings}>
            <Field labelEn="Week Starts On" labelAr="بداية الأسبوع">
              <select value={s.weekStartDay}
                ...
                className="rounded-lg border border-[var(--foreground)]/[0.08] bg-transparent px-3 py-1.5 text-xs focus:outline-none focus:border-[var(--color-primary)]/40">
                {[
            ...
            <Field labelEn="Hydration Target (glasses)" labelAr="هدف الترطيب (أكواب)">
              <input type="number" min={1} max={20} value={s.hydrationTarget}
                ...
                className="w-20 rounded-lg border border-[var(--foreground)]/[0.08] bg-transparent px-3 py-1.5 text-xs text-center focus:outline-none focus:border-[var(--color-primary)]/40" />
            </Field>
          ...
          <Section titleEn="Data Management" titleAr="إدارة البيانات" icon={Shield}>
            <div className="flex gap-3">
              <button onClick={handleExport}
                className="flex-1 flex items-center justify-center gap-2 rounded-xl border border-[var(--foreground)]/[0.08] py-2.5 text-xs font-medium hover:bg-[var(--foreground)]/[0.04] transition-colors">
            ...
            {showImport && (
              <div className="space-y-2">
                <textarea
                  ...
                  rows={4}
                  className="w-full rounded-xl border border-[var(--foreground)]/[0.08] bg-transparent px-3 py-2 text-xs font-mono focus:outline-none focus:border-[var(--color-primary)]/40 resize-none"
                />
                <div className="flex gap-2 justify-end">
                  <button onClick={() => setShowImport(false)} className="px-3 py-1.5 rounded-lg text-xs text-[var(--foreground)]/50">{isAr ? 'إلغاء' : 'Cancel'}</button>
            ...
            <div className="pt-2 border-t border-[var(--foreground)]/[0.06]">
              {!showConfirmReset ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => setShowConfirmReset(false)}
                    className="px-3 py-1 rounded-lg text-xs text-[var(--foreground)]/50">{isAr ? 'إلغاء' : 'Cancel'}</button>
                </div>
              )}
            </div>
          </Section>
        ...
        <motion.div variants={fadeUp} custom={4}>
          <div className="rounded-2xl border border-[var(--foreground)]/[0.06] bg-[var(--foreground)]/[0.02] p-5">
            <h3 className="text-xs font-semibold text-[var(--foreground)]/40 mb-3 uppercase tracking-wider">{isAr ? 'إحصائيات البيانات' : 'Data Stats'}</h3>
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={i} className="text-center">
                  ...
                  <p className="text-[10px] text-[var(--foreground)]/40">{isAr ? stat.labelAr : stat.labelEn}</p>
                </div>
              ))}
            </div>
          </div>
