          ...
          <>
            ...
            >
              <HabitDetail habit={detailHabit} onClose={() => setDetailHabit(null)} onEdit={() => { setDetailHabit(null); openEdit(detailHabit); }} onViewFull={() => { setFullCalendarHabit(detailHabit); }} allHabits={store.habits.filter(h => !h.archived && (filterCategory === 'all' || h.category === filterCategory))} onNavigate={(h) => setDetailHabit(h)}
                onArchive={() => store.toggleHabitArchive(detailHabit.id)}
                onDelete={() => {
                  const id = detailHabit.id;
                  // Remove from local state completely (not just archive)
                  store.updateHabit(id, {}); // trigger a state update
                  // True delete from DB (habit + logs + timer sessions)
                  ...
                  if (raw) {
                    try {
                      ...
                      state.habits = (state.habits || []).filter((h: Habit) => h.id !== id);
                      state.habitLogs = (state.habitLogs || []).filter((l: HabitLog) => l.habitId !== id);
                      localStorage.setItem('habits-app-state', JSON.stringify(state));
                    } catch {}
                  }
