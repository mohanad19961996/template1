...
import { ArrowLeft, ArrowRight, Plus, X, HelpCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-1">
      ...
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full h-9 px-3 rounded-lg text-[13px] font-medium outline-none cursor-pointer"
        style={{
          border: "1.5px solid rgba(var(--color-primary-rgb) / 0.12)",
          background: "rgba(var(--color-primary-rgb) / 0.02)",
          transition: "border-color 0.2s ease",
...
export function FaqEditorContent() {
  ...
  const { config, updateFaqItems } = useSiteConfig();
  ...
  const categoryOptions = [
    ...
    { value: "pricing", label: isAr ? "الأسعار" : "Pricing" },
  ];

  const update = (index: number, updates: Partial<FaqItem>) => {
  ...
  const removeItem = (index: number) => {
    updateFaqItems(items.filter((_, i) => i !== index));
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight">
                {isAr ? "محرر الأسئلة الشائعة" : "FAQ Editor"}
              </h1>
              <p className="text-[12px] mt-0.5" style={{ opacity: 0.4 }}>
                {isAr ? "إدارة الأسئلة المتكررة" : "Manage frequently asked questions"}
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <TextInput
                    ...
                    onChange={(v) => update(index, { questionEn: v })}
                    placeholder="How does it work?"
                  />
                  <TextInput
                    label={isAr ? "السؤال (عربي)" : "Question (Arabic)"}
                    value={item.questionAr}
                    onChange={(v) => update(index, { questionAr: v })}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <TextArea
                    ...
                    value={item.answerEn}
                    onChange={(v) => update(index, { answerEn: v })}
                    placeholder="Here's how it works..."
                  />
                  <TextArea
                    label={isAr ? "الإجابة (عربي)" : "Answer (Arabic)"}
                    value={item.answerAr}
                    onChange={(v) => update(index, { answerAr: v })}
                    placeholder="إليك كيف يعمل..."
                    dir="rtl"
                  />
                </div>
              </div>
