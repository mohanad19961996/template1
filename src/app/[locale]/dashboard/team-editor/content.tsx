...
export function TeamEditorContent() {
  ...
  const { config, updateTeamMembers } = useSiteConfig();
  const members = config.teamMembers ?? [];
  const BackArrow = isAr ? ArrowRight : ArrowLeft;

  const update = (index: number, updates: Partial<TeamMember>) => {
  ...
  const removeMember = (index: number) => {
    updateTeamMembers(members.filter((_, i) => i !== index));
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight">
                {isAr ? "محرر الفريق" : "Team Editor"}
              </h1>
              <p className="text-[12px] mt-0.5" style={{ opacity: 0.4 }}>
                {isAr ? "إدارة أعضاء الفريق" : "Manage team members"}
              </p>
            </div>
          </div>
        </div>
          ...
          {members.map((member, index) => (
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <TextArea
                    label={isAr ? "السيرة (إنجليزي)" : "Bio (English)"}
                    value={member.bioEn}
                    onChange={(v) => update(index, { bioEn: v })}
                    placeholder="Short bio..."
                  />
                  <TextArea
                    label={isAr ? "السيرة (عربي)" : "Bio (Arabic)"}
                    value={member.bioAr}
                    onChange={(v) => update(index, { bioAr: v })}
